...
- **Desenhar textura crua (`.tga`) no HUD 2D tem duas armadilhas já pagas caro** (ver `client/ammo.cpp::RTN_DrawBoxIcon` e os dois casos que já funcionavam, `hud_radio.cpp`/`hud_textwindow.cpp`):
  1. **`GL_Bind` do render API é cacheado.** Ele compara com o estado que o engine acha que está ligado e **não emite `glBindTexture`** se achar que já está. Só que o renderer do mod (`client/render/`) liga textura direto por `pgl*` durante a cena, então esse cache chega no HUD dessincronizado do GL real - o bind vira no-op e o quad sai pintado com a **última textura ligada de fato** (sintoma real observado: o ícone da arma virou a arte do menu de configuração depois de abrir o menu). Contorno: ligar uma textura conhecida antes (`FIND_TEXTURE( "*white" )`) pra forçar o cache a mudar de valor.
  2. **Não setar/resetar estado GL por item.** Setar `RenderMode`/`CullFace` a cada ícone gerava `GL_INVALID_ENUM (at R_RenderScene:976)` - e só aparecia com **2+ ícones**, porque o churn se repetia. `R_RenderScene:976` é só um `GL_CheckForErrors()`; `glGetError` é sticky, então o erro nasce no HUD 2D do frame anterior, não ali. Padrão correto (o que `hud_radio`/`hud_textwindow` já faziam): estado setado **uma vez** em volta do lote inteiro, `GL_Blend(GL_TRUE)`/`GL_Blend(GL_FALSE)` em volta, **não encostar em `CullFace`**, e **não intercalar** TriAPI com o 2D do engine (`FillRGBA`/`DrawHudString`) - desenhar os quads todos primeiro, o texto depois.
  3. **NUNCA chamar `FREE_TEXTURE` numa textura carregada por nome.** Foi esta a causa raiz do `GL_INVALID_ENUM` que custou várias rodadas. O engine **cacheia textura por nome**: `LOAD_TEXTURE("gfx/vgui/ammo/640_weapon_x.tga")` devolve o **mesmo índice** pra quem chamar. O `CHudWeaponBox` liberava a sua no `Reset()` e a cada troca de arma; a barra de seleção (`WEAPON::hBoxTex`) guardava o mesmo índice pra sempre. Trocar de arma → `FREE_TEXTURE` → o handle da barra ficava **pendurado** → `GL_Bind` dele dava `GL_INVALID_ENUM` **todo frame**, e como o bind falhava o quad saía com a textura anterior ainda ligada (daí o quadrado branco). Isso explica a assinatura observada: erro só ao **girar a rodinha** e só com **2+ armas** - com uma arma só nunca há troca, logo nunca há free. Regra: largar o handle (`TextureHandle::Null()`) e deixar o engine dono; recarregar pelo nome devolve o mesmo índice, então não duplica nada na GPU.
  - Existe `rtn_hud_selectbar_gldebug` (default 0) que checa erro de GL **depois de cada chamada** do desenho de ícone, cada uma numa linha diferente - como o `GL_CheckForErrors` imprime `(at <função>:<linha>)`, ele aponta a chamada culpada em vez de só acusar o `R_RenderScene:976`. Autodesliga depois de algumas linhas pra não inundar o console. Foi ele que fechou este caso.
  - Existe `rtn_hud_selectbar_icons` (default 1) pra desligar os ícones da barra em jogo: em 0 a barra não encosta na TriAPI, o que isola se um erro de GL vem daqui ou de outro lugar.
